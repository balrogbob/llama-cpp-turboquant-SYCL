...
bool llama_kv_cache_iswa::state_supports_append(llama_seq_id seq_id, llama_state_seq_flags flags) const {
    if ((flags & LLAMA_STATE_SEQ_FLAGS_PARTIAL_ONLY) == 0) {
        return false;
    }

    return kv_swa->state_supports_append(seq_id, flags);
}

size_t llama_kv_cache_iswa::state_write_from(llama_io_write_i & io, llama_seq_id seq_id, llama_state_seq_flags flags, llama_pos from_pos) const {
    if (!state_supports_append(seq_id, flags)) {
        return 0;
    }

    return kv_swa->state_write_from(io, seq_id, flags, from_pos);
}

size_t llama_kv_cache_iswa::state_read_append(llama_io_read_i & io, llama_seq_id seq_id, llama_state_seq_flags flags) {
    if (!state_supports_append(seq_id, flags)) {
        return 0;
    }

    return kv_swa->state_read_append(io, seq_id, flags);
}
