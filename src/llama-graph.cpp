...
static void print_mask(const ggml_fp16_t * data, int64_t n_tokens, int64_t n_kv, int64_t n_swa, llama_swa_type swa_type) {
    LLAMA_LOG_DEBUG("%s: === Attention mask ===\n", __func__);
    const char * swa_type_str = "unknown";
    ...
    for (int i = 0; i < std::min((int64_t)20, n_tokens); ++i) {
        LLAMA_LOG_DEBUG(" %2d ", i);
        for (int j = 0; j < std::min((int64_t)20, n_kv); ++j) {
            float val = ggml_fp16_to_fp32(data[i * n_kv + j]);
            if (val == -INFINITY) {
                LLAMA_LOG_DEBUG(" ∞");
            } else {
                LLAMA_LOG_DEBUG(" 0");
...
void llm_graph_input_attn_no_cache::set_input(const llama_ubatch * ubatch) {
    ...
    const auto fill_mask = [&](auto * data, int n_swa, llama_swa_type swa_type) {
        ...
        const auto set_val = [&](float val) -> mask_t {
            if constexpr (std::is_same_v<mask_t, ggml_fp16_t>) {
        ...
        for (int i1 = 0; i1 < n_tokens; ++i1) {
            ...
            for (int i0 = 0; i0 < n_tokens; ++i0) {
                ...
                data[idst + i0] = set_val(hparams.use_alibi ? -std::abs(p0 - p1) : 0.0f);
            }
        }
    };

    {
        ...
        ggml_fp16_t * data = (ggml_fp16_t *) self_kq_mask->data;

        std::fill(data, data + ggml_nelements(self_kq_mask), ggml_fp32_to_fp16(-INFINITY));
    ...
    if (hparams.swa_type != LLAMA_SWA_TYPE_NONE) {
        ...
        ggml_fp16_t * data = (ggml_fp16_t *) self_kq_mask_swa->data;

        std::fill(data, data + ggml_nelements(self_kq_mask_swa), ggml_fp32_to_fp16(-INFINITY));
...
void llm_graph_result::set_inputs(const llama_ubatch * ubatch) {
    ...
    const uint32_t n_kv_valid = params.mctx ? params.mctx->get_n_kv_valid() : 0;
    for (auto * t : t_fattn) {
        ggml_flash_attn_ext_set_implicit_causal(t, params.use_implicit_kq_mask, n_kv_valid);
    }
}
...
llm_graph_context::llm_graph_context(const llm_graph_params & params) :
    ...
    use_implicit_kq_mask(params.use_implicit_kq_mask),
    cb_func          (params.cb),
    res              (params.res),
    ctx0             (res->get_ctx()),
    gf               (res->get_gf()) {
...
ggml_tensor * llm_graph_context::build_attn_mha(
    ...
    if (use_flash_attn) {
        ...
        ggml_tensor * kq_mask_fa = use_implicit_kq_mask ? nullptr : kq_mask;

        cur = ggml_flash_attn_ext(ctx0, q, k, v, kq_mask_fa, kq_scale, hparams.f_max_alibi_bias,
                                  hparams.attn_soft_cap ? hparams.f_attn_logit_softcapping : 0.0f);
        cb(cur, LLAMA_TENSOR_NAME_FATTN, il);

        ggml_flash_attn_ext_add_sinks(cur, sinks);
        ggml_flash_attn_ext_set_prec (cur, GGML_PREC_F32);
        ggml_flash_attn_ext_set_implicit_causal(cur, use_implicit_kq_mask, mctx ? mctx->get_n_kv_valid() : 0);
        res->t_fattn.push_back(cur);
...
llm_graph_input_attn_kv * llm_graph_context::build_attn_inp_kv() const {
    ...
    auto inp = build_attn_inp_kv_impl(ctx0, ubatch, hparams, cparams, mctx_cur, use_implicit_kq_mask);
...
llm_graph_input_attn_k * llm_graph_context::build_attn_inp_k() const {
    ...
    auto inp = build_attn_inp_k_impl(ctx0, ubatch, hparams, cparams, mctx_cur, use_implicit_kq_mask);
...
llm_graph_input_attn_cross * llm_graph_context::build_attn_inp_cross() const {
    ...
    inp->cross_kq_mask = ggml_new_tensor_4d(ctx0, cparams.flash_attn ? GGML_TYPE_F16 : GGML_TYPE_F32, n_enc, n_tokens, 1, 1);
...
llm_graph_input_mem_hybrid * llm_graph_context::build_inp_mem_hybrid() const {
    ...
    auto inp_attn = build_attn_inp_kv_impl(ctx0, ubatch, hparams, cparams, mctx_cur->get_attn(), use_implicit_kq_mask);
...
llm_graph_input_mem_hybrid_k * llm_graph_context::build_inp_mem_hybrid_k() const {
    ...
    auto inp_rs   = build_rs_inp_impl     (ctx0, ubatch, mctx_cur->get_recr());
    auto inp_attn = build_attn_inp_k_impl(ctx0, ubatch, hparams, cparams, mctx_cur->get_attn(), use_implicit_kq_mask);
